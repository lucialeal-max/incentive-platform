...
const AVATAR_COLOR: Record<string, string> = {
  AG: "bg-purple-600", CL: "bg-indigo-600", MP: "bg-blue-600",
  JT: "bg-teal-600",  SR: "bg-orange-600", DV: "bg-green-600",
};
...
export function SessionHeader({ adminNav, employeeNav }: { adminNav?: boolean; employeeNav?: boolean }) {
  ...
  const isAdminArea = ["/dashboard","/objectives","/exceptions","/payout","/export"].some(p => path.startsWith(p));

  if (!user) return <header className="bg-white border-b border-gray-100 sticky top-0 z-10 h-14" />;

  const links = adminNav ? ADMIN_LINKS : EMP_LINKS;
    ...
    <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {user.isAdmin && (
            isAdminArea ? (
              <a href="/my-objectives" className="text-xs px-3 py-1.5 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 transition flex items-center gap-1.5">
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                Vista empleado
              </a>
            ) : (
              <a href="/dashboard" className="text-xs px-3 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-2.5 pl-3 border-l border-gray-100">
            ...
            <button onClick={handleLogout} className="text-xs text-gray-400 hover:text-gray-600 ml-1 transition" title="Salir">
