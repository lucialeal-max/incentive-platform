import { DEMO_INSTANCES, DEMO_PROFILES, DEMO_OBJECTIVES } from "@/lib/demo-data";
...
export default function DashboardPage() {
  ...
  const totalPayout = instances.filter(i => i.status === "approved").reduce((s, i) => s + (i.bonusAmount ?? 0), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        <div className="flex rounded-full overflow-hidden h-3 gap-0.5">
          <div className="bg-emerald-400" style={{ width: (approved/total*100) + "%" }} />
          <div className="bg-amber-400" style={{ width: (inReview/total*100) + "%" }} />
          <div className="bg-red-400" style={{ width: (rejected/total*100) + "%" }} />
        </div>
        <div className="flex gap-4 mt-2 text-xs text-gray-500">
          <span><span className="inline-block w-2 h-2 rounded-full bg-emerald-400 mr-1"/>Aprobados {approved}</span>
          <span><span className="inline-block w-2 h-2 rounded-full bg-amber-400 mr-1"/>En revisión {inReview}</span>
          <span><span className="inline-block w-2 h-2 rounded-full bg-red-400 mr-1"/>Rechazados {rejected}</span>
        </div>
      </div>
      <ExceptionsQueue />
    </div>
