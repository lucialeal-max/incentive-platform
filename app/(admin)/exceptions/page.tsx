  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">{exceptions.length} casos pendientes</p>
      </div>
          ...
          return (
            <div key={inst.id} className="bg-white rounded-2xl border border-amber-100 p-5">
              ...
              <div className="mt-3 space-y-1">
                {inst.conditionResults.map((cr, i) => (
                  <div key={i} className="flex items-center gap-2 text-sm">
                    <span className={cr.passed ? "text-emerald-500" : "text-red-400"}>{cr.passed ? "✓" : "✗"}</span>
              ...
              <div className="flex gap-2 mt-4">
                <button className="flex-1 text-sm py-2 px-4 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition font-medium">✓ Aprobar</button>
                <button className="flex-1 text-sm py-2 px-4 border border-gray-200 text-gray-700 rounded-xl hover:bg-gray-50 transition font-medium">✗ Rechazar</button>
              </div>
            </div>
          );
        })}
